...
mutable struct WiSARDRegressor
    n_bits::Int
    n_tics::Int
    random_state::Int
    mapping::String
...
ScikitLearnBase.@declare_hyperparameters(WiSARDRegressor,
                         [:n_bits, :n_tics, :random_state, :mapping, :code, :debug])

ScikitLearnBase.is_classifier(::WiSARDRegressor) = false   # not required for transformers
...
function ScikitLearnBase.fit!(dt::WiSARDRegressor, X, y)
    n_samples, n_features = size(X)
    dt.retina_size = dt.n_tics * n_features
    dt.n_rams = dt.retina_size % dt.n_bits == 0 ? ÷(dt.retina_size,dt.n_bits) : ÷(dt.retina_size,dt.n_bits + 1)
...
function ScikitLearnBase.predict(dt::WiSARDRegressor, X)
    n_samples, _ = size(X)
